...
### some results
The following are some names generated by the bigram model:
- tnivchh, bstudy, casman, njseyan, ph, tcdi, mu, rekhjwvfarunxn, a, artygbyqjagy, bbshaik, kay, oce\
These are of course not very good names which is to be expected since the model is just a simple one layer NN.

The following are some names generated by the MLP:
- tios, kidanni, zah, sibi, saki, miana, kiyia, priya, abbruya, xolefraccon, ina, jbina, wrik, nohan, jarya, maxysen, gilee, nee, amayan, iana\
In contrast to the bigram model, they seem to be much more similar to typical names.
